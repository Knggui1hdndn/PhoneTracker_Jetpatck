...
object Permission {
    fun hasPermissionLocation(context: Context): Boolean {
        return (context.checkSelfPermission(Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED) || (context.checkSelfPermission(
    ...
    fun hasEnableGPS(context: Context): Boolean {
     ...
     private fun turnOnGPS(context: Context,call:(Boolean)->Unit) {
          val request = LocationRequest.Builder(1000).setPriority(Priority.PRIORITY_HIGH_ACCURACY).setMaxUpdateDelayMillis(1000).build()
